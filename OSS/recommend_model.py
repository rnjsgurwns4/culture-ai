...
surprise_df = pd.read_csv("surprise_recommendations.csv")  # Surprise 모델 결과
...
def get_hybrid_recommendations(user_id: int, top_n_each: int = 3):
    ...
    # Surprise 추천 추출
    surprise_top = (
        surprise_df[surprise_df['member_id'] == user_id]
        .sort_values(by='score', ascending=False)
        .head(top_n_each)['content_detail_id']
        .tolist()
    )

    return xgb_top + surprise_top  # 앞 3개는 xgb, 뒤 3개는 surprise
